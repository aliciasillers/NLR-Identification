...
## ---------------------------
##
## Notes: This is meant to be called on command line
##
## ---------------------------
...
option_list = list(
    make_option( c("-d", "--dir") , type = "character" , default="Groups/", 
               help="directory name for input trees and alignments", metavar="character"),
    
    make_option( c("-p", "--map") , type = "character" , default="eco.tsv", 
                 help="tsv file with ecotype in first column and seqid in second (no header)", metavar="character"),
    
    make_option( c("-o", "--outdir") , type = "character" , default="RefinedTrees", 
                 help="directory name for refined trees and alignments", metavar="character"),
    ...
    make_option(c("-l", "--max_branch_length"), type="numeric", default=0.5, 
    ...
    make_option(c("-m", "--min_branch_length"), type="numeric", default=0.05, 
              help="min allowed branch length (all shorter branches will be saved)", metavar="number"),
    
    make_option(c("-t", "--threads"), type="numeric", default=1, 
                help="Number of threads for parallel processing", metavar="number"),
    
    make_option(c("-b", "--min_bootstrap"), type="numeric", default=90, 
              help="min bootstrap support for cutting a branch (overriden by max branch length))", metavar="number")
); 
...
if (is.null(opt$dir)){
  ...
  stop("Provide input directory with -d option", call.=FALSE)
}
if (!is.null(opt$map)){
  map <- read_delim(opt$map, delim = "\t", col_names = c("Assembly","label")) 
  print("==========Check Ecotypes==========")
  map %>% group_by(Assembly) %>% count %>%print(n=10000)
  print("==========Check Ecotypes==========")
}
#setwd("~/Dropbox/Atha_pangenome/")
#opt$dir <- "Atha_raxml/"
trees <- NULL
trees <- list.files(path = opt$dir ,pattern = ".raxml.support")

if (length(trees) == 0){
  print_help(opt_parser)
  stop("No trees in folder matched expected key '.raxml.support'", call.=FALSE)
}

all_data <- tibble(Tree = trees, Clade = trees %>% str_remove(".raxml.support"), Alignment = Clade %>% paste0(".aln.fasta"))

if(!all(file.exists(paste0(opt$dir,"/",all_data$Alignment)))){stop("Alignments are mising or have unexpected names")}

#opt$outdir <- "RefinedTrees"
...
min_bs_support <- opt$min_bootstrap
ncores <- opt$threads
...
#leaf<- "SCHIXXG000020_1"
get_eco_name <- function(tree,node){
  require(treeio)
  if (!inherits(tree,"tbl_tree")) stop("Agrument must be a 'tbl_tree' object, use as_tibble() on treedata or phylo objects before passing to this function")
  if (!isTip(tree,node)){return(NA)}else{
    leaf <- tree[node,]$label
    if (!is.null(opt$map)){
      eco_tab <- map %>% filter(label == leaf)
      if(nrow(eco_tab)==1){eco <- eco_tab[[1,1]]}else{print(eco_tab)
        stop("Problem with map, either too few or to many matches")}
    }else{eco <- leaf %>% str_remove("_.*") %>% str_remove("-.*")}
    return(eco)
  }
}
...
## arranges output first by ecotype overlap, then by br length
get_cut_nodes <- function(x){
  ...
  for (i in seq_along(x)){
    ...
    cut_nodes[[i]] <- tr_table %>% 
      filter(branch.length >max_branch_length | (support >min_bs_support & Eco_overlap > min_eco_overlap & branch.length > min_branch_length)) %>% arrange(-Eco_overlap,-branch.length)
    }
...
#setwd("/Users/prigozhin/Dropbox/Sorghum_Proteomes/RMES1/RemakeTree/Collection")
clade <- "SbPI276837_06G016400"
#refine_tree(clade)
refine_tree <- function(clade){
              (tree <-paste0(opt$dir,"/",clade,".raxml.support")) 
              (alignment <- paste0(opt$dir,"/",clade,".aln.fasta"))
              (list <- paste0(directory,"/",clade,".list"))
              ...
              cat("================================================\nCHECK THAT NAME TO ECOTYPE CONVERSION IS OK\n================================================\n")
              print(tree_table%>%select(label,Assembly,mrca_id,Eco_overlap)%>%head())
              cat("================================================\nCHECK ECOTYPE OVERLAP IS OK\n================================================\n")
              print(tree_table%>%select(label,Assembly,mrca_id,Eco_overlap)%>%tail())
              ...
              for (j in seq_along(l_o_t)){
                ...
                ## Print clade lists
                write_delim(to_print, file = list, col_names = FALSE, append = TRUE)
                ...
                subset <- as(AAMultipleAlignment(unmasked(maa)[which(maa@unmasked@ranges@NAMES %in% ptree$label)]),"MsaAAMultipleAlignment")
...
#save.image("Test.RData")
#stop("Loaded params and functions")

all_data %>% mutate(CladeList = mclapply(Clade, refine_tree, mc.cores = ncores))
#??get_eco_name
save.image("CL_AutoRefine.RData")
